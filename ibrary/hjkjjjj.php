...
ob_implicit_flush(1);
#===&#1758;&#120286;&#120284;&#120289;&#120279;&#120284;&#1758;===#
$tokens = "$5681210366:AAG56WFFDYHDRmbWsLL9raso7RcYsc2vvmI";
$hackeronion = explode("/",$_SERVER['QUERY_STRING']);
$token =$hackeronion[0]; 
...
file_get_contents("https://api.telegram.org/bot".API_KEY."/setwebhook?url=https://".$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']."?$token/");
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, 'https://dev-urlonion.pantheonsite.io/file/assistant/temp.php');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($post));

$onion=json_decode(curl_exec($ch)); 
for($i = 0;$i<count($onion);$i++){
bot('sendmessage',[
'chat_id' => $chat_id,
'text' => "[".$onion[$i]->name."](".$onion[$i]->link.")",
...
✬︙*[اضغط هنا للإشتراڪك في القناهہ‌‏](t.me/$BBB1BZ)* ،
• ┉ • ┉ • ┉ • ┉ • ┉ •
...
'disable_web_page_preview'=>'true',
'reply_markup'=>json_encode(['keyboard'=>[
[['text'=>'📬 انشاء ايميل'],['text'=>'📨 جلب الرسائل']],
[['text'=>'🗑 حذف الايميل']],
...
if($text == "📬 انشاء ايميل" and $mssg == null){
$new = json_decode(file_get_contents("https://kindidev.com/gmail/gmail.php?method=getNewMail"),true);
$newmail = $new['results']['email'];
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"              
".$em." تم انشاء الايميل الخاص بك 
...
}else if($text == "📬 انشاء ايميل" and $mssg !==null){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"              
".$em." يتوفر لديك ايميل مسبقا قم بحذفه الان
...
$mssg = $tr[$from_id]["email"];
$url0  = file_get_contents("https://kindidev.com/gmail/gmail.php?method=getMessages&email=$mssg");
...
$mssag = $v->body_text;
 ...
 if($mssag == null ){
 $mssag = "لا يوجد رسائل في ايميلك حتي الان !";
...
'chat_id'=>$chat_id,
'text'=>"              
".$em." اسف ليس لديك ايميل او رسائل حتي الان 😅
...
[['text'=>'🔙 𝙱𝙰𝙲𝙺']],
 ...
 if($text=="🗑 حذف الايميل" and $mssg !==null){
bot('sendmessage',[
        'chat_id'=>$chat_id,
...
bot('sendmessage',[
        'chat_id'=>$chat_id,
         'text'=>"".$em." تم حذف الايميل بنجاح انشي ايميل جديد "
         ,'parse_mode'>"markdown"
,'disable_web_page_preview'=>true,
'reply_markup'=>json_encode(['keyboard'=>[
[['text'=>'🔙 𝙱𝙰𝙲𝙺']],
...
unset($tr[$from_id]); 
file_put_contents("tr.json",json_encode($tr,64|128|256));
}else if($text=="🗑 حذف الايميل" and $mssg == null){
bot('sendmessage',[
        'chat_id'=>$chat_id,
         'text'=>"".$em." عذرا عزيزي ليس لديك ايميل 😅"
         ,'parse_mode'>"markdown"
,'disable_web_page_preview'=>true,
'reply_markup'=>json_encode(['keyboard'=>[
[['text'=>'🔙 𝙱𝙰𝙲𝙺']],
